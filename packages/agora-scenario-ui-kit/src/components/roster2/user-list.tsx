...
import { ActionTypes } from '~components/roster2';
...
import { renderRows } from './infinite-scroll-list';
// import { renderRows } from './virtualized-list'
import PeopleRadioGroup from './people-radio-group';
...
export type StudentRosterProps = {
  ...
  onFetch?: (
    params?: Partial<{ nextId: number | null; count: number; type: '0' | '1' }>,
    reset?: boolean,
  ) => Promise<void>;
  hasMore?: boolean;
};
...
export const StudentRoster: React.FC<StudentRosterProps> = ({
  ...
  const StudentRow = ({ data, style }: { data: StudentRosterProfile; style?: any }) => (
    <Row className={'border-bottom-width-1'} key={data.uid} style={style}>
  ...
  //   const studentData = studentList[index];

  //   return <StudentRow style={style} data={studentData} />;
  // };

  // const rows = renderRows(studentList, ListRow)

  // infinite-scroll-list
  const rows = renderRows(
    studentList,
    (data) => <StudentRow data={data} key={data.uid} />,
    onFetch,
    hasMore,
  );
  ...
  useEffect(() => {
    const defaultType = '0';
    // load by default type
    onFetch({ type: defaultType, nextId: null }, true);
  }, []);
      ...
      <div className="agora-board-resources roster-user-list-wrap">
        ...
        <div className="roster-container">
          ...
          <Table className="roster-table">
            ...
            <Table className="table-container">{rows}</Table>
          </Table>
        </div>
      </div>
    </DraggableContainer>
