...
import { Confirm } from '~capabilities/containers/dialog';
...
export const CloudDriverContainer: React.FC<CloudDriveContainerProps> = observer(({ id }: any) => {
  const { setTool, room } = useBoardContext();

  const { openCloudResource, refreshCloudResources, cancelUpload, removeMaterialList, doUpload } =
    useCloudDriveContext();

  const { checked, removeDialog, addDialog } = useUIStore();
  ...
  const handleDelete = async () => {
    addDialog(Confirm, {
      ...
      title: transI18n('confirm.delete'),
      content: transI18n('cloud.deleteTip'),
    });
  };
        ...
        <Tabs activeKey={activeKey} onChange={handleChange}>
          ...
          <TabPane tab={transI18n('cloud.personalResources')} key="2">
            <Row style={{ paddingLeft: 19 }} className="btn-group margin-gap">
              ...
              <Button disabled={!checked} type="ghost" onClick={handleDelete}>
              ...
              {showUploadModal ? (
                <Modal
                  ...
                  <Loading
                    ...
                    hasLoadingGif={false}
                    noCloseBtn={true}
                    uploadItemList={[{ ...uploadFileInfo, currentProgress }]}
                  />
                </Modal>
              ) : (
                ''
