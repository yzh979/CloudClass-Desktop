...
export class EduSDKApi extends ApiBase {
  ...
  updateRtmInfo(info: { rtmToken: string; rtmUid: string }) {
  ...
  async reportCameraState(payload: {
    roomUuid: string;
    userUuid: string;
    state: number;
  }): Promise<any> {
  ...
  async updateClassState(params: { roomUuid: string; state: number }) {
    const res = await this.fetch({
  ...
  async muteStudentChat(params: { roomUuid: string; userUuid: string }) {
    const res = await this.fetch({
  ...
  async unmuteStudentChat(params: { roomUuid: string; userUuid: string }) {
    const res = await this.fetch({
      url: `/v2/rooms/${params.roomUuid}/users/${params.userUuid}/mute`,
      method: 'PUT',
      data: {
  ...
  async startHandsUp(params: { roomUuid: string; toUserUuid: string }) {
    const res = await this.fetch({
  ...
  async kickOutOnce(params: { roomUuid: string; toUserUuid: string }) {
    const res = await this.fetch({
  ...
  async kickOutBan(params: { roomUuid: string; toUserUuid: string }) {
    const res = await this.fetch({
      url: `/v2/rooms/${params.roomUuid}/users/${params.toUserUuid}/exit`,
      method: 'POST',
      data: {
