...
export class RoomStore extends SimpleInterval {
    ...
    return this.roomChatMessages
      .filter(
        (item: ChatMessage, index: number) =>
          this.roomChatMessages.findIndex(
            (predicateValue) => predicateValue.messageId === item.messageId,
          ) === index,
      )
  ...
  async getConversationList(data: { nextId: string; sort: number }) {
    try {
      ...
      const error = GenericErrorWrapper(err);
      this.appStore.fireToast('toast.failed_to_get_conversations', {
        reason: error,
      });
      BizLogger.warn(`${error}`);
    }
  ...
  addConversationChatMessage(args: any, conversation: any) {
    ...
    if (!chatConversation) {
      ...
      if (
        chatConversation.messages.findIndex((msg) => {
  ...
  async sendReward(userUuid: string, reward: number) {
    try {
      ...
      this.appStore.fireToast('toast.failed_to_send_reward');
      const error = GenericErrorWrapper(err);
      BizLogger.warn(`${error}`);
    }
  }
  ...
  chatIsBanned(isStudentChatAllowed: boolean) {
    ...
    if (!isFirstLoad() && this.isStudentChatAllowed !== isStudentChatAllowed) {
      if (this.isStudentChatAllowed) {
        this.appStore.fireToast('toast.chat_enable');
      } else {
        this.appStore.fireToast('toast.chat_disable');
      }
    }
        ...
        case EduClassroomStateEnum.beforeStart:
          ...
          [5, 3, 1].forEach((min) => {
            if (dDuration.minutes() === min && dDuration.seconds() === 0) {
              this.appStore.fireToast('toast.time_interval_between_start', {
                reason: duration,
              });
            }
          });
  ...
  isBigClassStudent(): boolean {
    ...
    return (
      +this.roomInfo.roomType === EduRoomType.SceneTypeBigClass &&
      userRole === EduRoleTypeEnum.student
    );
  }
  ...
  pluginRoomProperties(extApp: IAgoraExtApp) {
    const props = get(
      this.roomProperties,
      `extApps.${escapeExtAppIdentifier(extApp.appIdentifier)}`,
    );
    return props;
  }
  ...
  async join() {
    try {
      ...
      roomManager.on('seqIdChanged', (evt: any) => {
        ...
        this.appStore.updateSeqId({
          current: evt.curSeqId,
          latest: evt.latestSeqId,
        });
      });
      // 本地用户更新
      ...
      roomManager.on('local-stream-removed', async (evt: any) => {
        ...
        await this.sceneStore.mutex.dispatch<Promise<void>>(async () => {
          ...
          try {
            const tag = uuidv4();
            BizLogger.info(
              `[demo] tag: ${tag}, [${Date.now()}], handle event: local-stream-removed, `,
              JSON.stringify(evt),
            );
            if (evt.type === 'main') {
              ...
              if (cause && cause.cmd === 501) {
                ...
                this.appStore.fireToast(`roster.close_student_co_video`, {
                  teacher: role,
                });
              }
              BizLogger.info(
      ...
      roomManager.on('local-stream-updated', async (evt: any) => {
        ...
        await this.sceneStore.mutex.dispatch<Promise<void>>(async () => {
          ...
          if (evt.type === 'main') {
            ...
            BizLogger.info(
              `[demo] local-stream-updated tag: ${tag}, time: ${Date.now()} local-stream-updated, main stream `,
            ...
            if (localStream && localStream.state !== 0) {
              if (causeCmd === 501) {
                ...
                const role = roleMap[operator.userRole] ?? 'unknown';
                this.appStore.fireToast(`roster.open_student_co_video`, {
                  teacher: role,
                });
              }
              BizLogger.info(
                `[demo] local-stream-updated tag: ${tag}, time: ${Date.now()} local-stream-updated, main stream is online`,
                ' _hasCamera',
                this.sceneStore._hasCamera,
                ' _hasMicrophone ',
                this.sceneStore._hasMicrophone,
                this.sceneStore.joiningRTC,
              );
              if (this.sceneStore._cameraEduStream) {
                if (!!localStream.stream.hasVideo !== !!this.sceneStore._cameraEduStream.hasVideo) {
                  ...
                  if (causeCmd !== 501) {
                    ...
                    ![DeviceStateEnum.Disabled].includes(
              ...
              BizLogger.info(
                `[demo] tag: ${tag}, seq[${
                  evt.seqId
                }], time: ${Date.now()} local-stream-updated, main stream is online`,
                ' _hasCamera',
                this.sceneStore._hasCamera,
                ' _hasMicrophone ',
                this.sceneStore._hasMicrophone,
                this.sceneStore.joiningRTC,
                ' _eduStream',
                JSON.stringify(this.sceneStore._cameraEduStream),
              );
              if (this.sceneStore.joiningRTC) {
                if (this.sceneStore.cameraEduStream.hasVideo) {
                  await this.sceneStore.unmuteLocalCamera();
                  BizLogger.info(
                    `[demo] local-stream-updated tag: ${tag}, seq[${
                      evt.seqId
                    }], time: ${Date.now()}  after openCamera  local-stream-updated, main stream is online`,
                    ' _hasCamera',
                    this.sceneStore._hasCamera,
                    ' _hasMicrophone ',
                    this.sceneStore._hasMicrophone,
                  ...
                  BizLogger.info(
                    `[demo] local-stream-updated tag: ${tag}, seq[${
                      evt.seqId
                    }], time: ${Date.now()}  after closeCamera  local-stream-updated, main stream is online`,
                    ' _hasCamera',
                    this.sceneStore._hasCamera,
                    ' _hasMicrophone ',
                    this.sceneStore._hasMicrophone,
                ...
                if (this.sceneStore.cameraEduStream.hasAudio) {
                  ...
                  BizLogger.info(
                    `[demo] local-stream-updated tag: ${tag}, seq[${
                      evt.seqId
                    }], time: ${Date.now()} after openMicrophone  local-stream-updated, main stream is online`,
                    ' _hasCamera',
                    this.sceneStore._hasCamera,
                    ' _hasMicrophone ',
                    this.sceneStore._hasMicrophone,
                  ...
                  BizLogger.info(
                    `[demo] local-stream-updated tag: ${tag}, seq[${
                      evt.seqId
                    }], time: ${Date.now()}  after closeMicrophone  local-stream-updated, main stream is online`,
                    ' _hasCamera',
                    this.sceneStore._hasCamera,
                    ' _hasMicrophone ',
                    this.sceneStore._hasMicrophone,
              ...
              BizLogger.info(
                'reset camera edu stream',
                JSON.stringify(localStream),
                localStream && localStream.state,
              );
              this.sceneStore._cameraEduStream = undefined;
            }
          }
          ...
          if (evt.type === 'screen') {
            if (this.roomInfo.userRole === EduRoleTypeEnum.teacher) {
              ...
              BizLogger.info(
                'local-stream-updated getLocalScreenData#screenStream ',
                JSON.stringify(screenStream),
              );
              if (screenStream && screenStream.state !== 0) {
                ...
                BizLogger.info(
                  'local-stream-updated reset screen edu stream',
                  screenStream,
                  screenStream && screenStream.state,
                );
          ...
          BizLogger.info(
            `[demo] local-stream-updated tag: ${tag}, seq[${
              evt.seqId
            }], time: ${Date.now()} local-stream-updated emit done`,
            evt,
          );
          BizLogger.info(
            `[demo] local-stream-updated tag: ${tag}, seq[${
      ...
      let encryptionConfig = get(
        this.appStore.params.config,
        'mediaOptions.encryptionConfig',
        null,
      );
      if (encryptionConfig) {
      ...
      if (canPublishRTC(localStreamData, sceneType)) {
        ...
        try {
          ...
          if (this.appStore.isNotInvisible) {
            this.appStore.fireToast('toast.media_method_call_failed', {
              reason: `${err.message}`,
            });
          }
          const error = GenericErrorWrapper(err);
      ...
      const error = GenericErrorWrapper(err);
      reportService.reportElapse('joinRoom', 'end', {
        result: false,
  ...
  handleCause(
    ...
    if (cmd === 501) {
      ...
      this.appStore.activeExtApps.forEach((app) => {
        let oldProps = get(
          oldRoomProperties,
          `extApps.${escapeExtAppIdentifier(app.appIdentifier)}`,
        );
        let newProps = get(
          newRoomProperties,
          `extApps.${escapeExtAppIdentifier(app.appIdentifier)}`,
