...
export class SceneStore extends SimpleInterval {
  ...
  get screenShareStreamList() {
    return this._streamList.filter(
  ...
  async sendUnmuteApply(source: 'video' | 'audio', userUuid: string) {
    ...
    await this.roomManager.userService.sendUserChatMessage(message, {
      userUuid,
    } as EduUser);
  }
  ...
  async startNativeScreenShareBy(shareId: any, type?: ScreenShareType) {
    try {
      ...
      this.waitingShare = false;
      this.appStore.fireToast('toast.failed_to_initiate_screen_sharing', {
        reason: `${err.message}`,
      });
    }
  }
  ...
  get openingCamera() {
    return this.roomInfo.userRole === EduRoleTypeEnum.teacher
      ? this.openingTeacherCamera
      : this.openingStudentCamera;
  }

  get closingCamera() {
    return this.roomInfo.userRole === EduRoleTypeEnum.teacher
      ? this.closingTeacherCamera
      : this.closingStudentCamera;
  }

  get loadingMicrophone() {
    return this.roomInfo.userRole === EduRoleTypeEnum.teacher
      ? this.loadingTeacherMicrophone
      : this.loadingStudentMicrophone;
  }
  ...
  async sendMuteLocalCamera() {
    if (this.cameraEduStream && this.cameraEduStream.hasVideo) {
      await this.roomManager?.userService?.updateMainStreamState({
        videoState: false,
      });
    }
  }
  ...
  async stopWebSharing() {
    try {
      ...
      this.appStore.fireToast('toast.failed_to_end_screen_sharing', {
        reason: `${err.message}`,
      });
    } finally {
      this.waitingShare = false;
  ...
  getStudentConfig() {
    ...
    if (
      roomType === EduRoomType.SceneTypeBigClass ||
      roomType === EduRoomType.SceneTypeMiddleClass
    ) {
      return {
        sceneType: roomType,
        userRole: 'audience',
    ...
    if (!user) {
      ...
      if (cameraState === 1) {
        const stream = this.streamList.find(
          (stream: EduStream) => stream.streamUuid === streamUuid,
        );
  ...
  getRemotePlaceHolderProps(userUuid: string, userRole: string) {
    ...
    if (
      this.queryRemoteCameraDeviceState(userUuid, stream.streamUuid) === DeviceStateEnum.Disabled
    ) {
      return {
        holderState: 'disabled',
        text: `placeholder.disabled`,
  ...
  queryVideoFrameIsNotFrozen(uid: number): boolean {
    ...
    if (isLocal) {
      if (
        this.appStore.mediaStore.localVideoState ===
  ...
  queryAudioIsNotFrozen(uid: number): boolean {
    ...
    if (isLocal) {
      if (
        this.appStore.mediaStore.localAudioState ===
  ...
  get teacherStream() {
    ...
    // 当远端是老师时
    const teacherStream = this.streamList.find(
      (it: EduStream) =>
        (it.userInfo.role as string) === 'host' &&
        it.userInfo.userUuid === this.teacherUuid &&
    ...
    if (this.teacher && teacherStream) {
      ...
      return {
        ...
        renderer: this.remoteUsersRenderer.find(
          (it: RemoteUserRenderer) => +it.uid === +teacherStream.streamUuid,
  ...
  private getStreamBy(userUuid: string): EduStream {
    return this.streamList.find(
      (it: EduStream) =>
        (get(it.userInfo, 'userUuid', 0) as string) === userUuid &&
        it.videoSourceType === EduVideoSourceType.camera,
    ) as EduStream;
  }
  ...
  async closeStream(userUuid: string, isLocal: boolean) {
    ...
    if (isLocal) {
      ...
      if (this.roomManager?.userService) {
        const targetStream = this.streamList.find(
          (it: EduStream) => it.userInfo.userUuid === userUuid,
        );
        // const targetUser = this.userList.find(
        //   (it: EduUser) => it.userUuid === targetStream?.userInfo.userUuid,
        // );
  ...
  async startRecording() {
    try {
      ...
      this.appStore.fireToast('toast.start_recording_failed', {
        reason: `${err.message}`,
      });
    }
  }
  ...
  async stopRecording() {
    try {
      ...
      const error = GenericErrorWrapper(err);
      this.appStore.fireToast('toast.failed_to_end_recording', {
        reason: `${error}`,
      });
    }
  }
