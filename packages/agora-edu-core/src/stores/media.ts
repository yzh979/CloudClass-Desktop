...
export class MediaStore {
  ...
  get localPacketLostRate() {
    return Math.max(
      this.localPacketLoss.audioStats.audioLossRate,
      this.localPacketLoss.videoStats.videoLossRate,
    );
  }
  ...
  constructor(appStore: EduScenarioAppStore) {
    ...
    this.mediaService.on('user-published', (evt: any) => {
      ...
      EduLogger.info(
        `[agora-apaas] [media#renderers] user-published ${this.mediaService.remoteUsersRenderer.map(
          (e) => e.uid,
        )}`,
      );
    ...
    this.mediaService.on('localVideoStats', (evt: any) => {
      ...
      BizLogger.info(
        'localVideoStats',
        ' encode fps ',
        evt.stats.encoderOutputFrameRate,
        ', freeze: ',
        freezeCount,
      );
      // autorun(() => {
    ...
    this.mediaService.on('remoteVideoStats', (evt: any) => {
      ...
      let { uid } = user;
      BizLogger.info(
        `remoteVideoStats ${uid}, decode fps ${stats.decoderOutputFrameRate}, freezeCount: ${stats.freezeCount}`,
      );
      // autorun(() => {
