...
export class BoardStore extends ZoomController {
  ...
  changeSceneItem(resourceUuid: string) {
    ...
    if (sceneIsChanged) {
      if (targetPath === "init") {
        if (currentPage === 0) {
          this.setScenePath(`/init`)
        } else {
          this.setScenePath(`/${currentPage}`)
        }
      }
      else if (targetPath === "screenShare") {
        this.setScenePath(`/${targetPath}`)
      }
      else {
        ...
        if (targetResource) {
          ...
          this.setScenePath(`${targetPath}/${scenePath}`)
        }
      }
    }
  ...
  async putCourseResource(resourceUuid: string) {
    ...
    if (resource) {
      ...
      if (sceneExists) {
        this.setScenePath(`/${resource.id}/${resource.scenes[0].name}`)
      } else {
