...
export class EduClassroomManager {
  ...
  constructor(payload: EduClassroomInitParams) {
    ...
    // rteReportService.initReportParams()
  ...
  async join(params: any) {
    try {
      // REPORT
      rteReportService.startTick('joinRoom', 'end');
      let data = await this._join(params);
      rteReportService.reportElapse('joinRoom', 'end', { result: true });
      return data;
    } catch (e) {
      rteReportService.reportElapse('joinRoom', 'end', {
        result: false,
  ...
  private async _join(params: any) {
    ...
    EduLogger.debug(
      `[EDU-STATE] [ClassRoom Manager] join classroom [prepareRoomJoin] ${this.roomUuid} success`,
    );
    if (this.rtmWrapper) {
      ...
      const onChannelMessageHandler = (evt: any) => {
        EduLogger.debug(
          '[EDU-STATE] [ClassRoom Manager] [rtm] ChannelMessage channelName',
          evt.channelName,
        );
        ...
        try {
          ...
          EduLogger.info(
            `[EDU-STATE] Raw ChannelMessage, res: `,
            JSON.stringify(res),
          ...
          EduLogger.error(
            '[EDU-STATE] onChannelMessageHandler, code: ',
            err.code,
            ' message: ',
            err.message,
          );
      ...
      await this.rtmWrapper.join(channel, observer, {
        channelName: this.roomUuid,
      });
