...
export class EduClassroomDataController {
  ...
  public setCurrentSeqId(v: number) {
    ...
    EduLogger.info(
      '[seqId] setCurrentSeqId, curSeqId: ',
      this._curSeqId,
      ' latestSeqId: ',
      this._latestSeqId,
    );
        ...
        if (this._curSeqId) {
          if (+seqId <= +this._curSeqId) {
            ...
            if (+seqId === this._curSeqId + 1) {
              ...
              const onFailure = () => {
                EduLogger.warn(
                  '[seqId] syncData missingSeqId failure',
                  this._curSeqId,
                  this._latestSeqId,
                );
              };
              this.syncData(false, 300).then(onSuccess).catch(onFailure);
              break;
            }
          }
        } else {
    ...
    for (let it of taskBuffer) {
      ...
      switch (cmd) {
        ...
        case EduChannelMessageCmdType.userListChanged: {
          const { onlineUsers, offlineUsers, onlineStreams, offlineStreams } =
          ...
          EduLogger.info(
            `[userListChanged] [${this._id}] before [${seqId}]#updateUsersStreams:  rawData `,
            data,
          );
          EduLogger.info(
            `[userListChanged] [${this._id}] before [${seqId}]#updateUsersStreams:  serializedData`,
            'onlineUsers ',
            onlineUsers,
            'offlineUsers ',
            offlineUsers,
        ...
        case EduChannelMessageCmdType.userStateUpdated: {
          EduLogger.info(
            `[userStateUpdated] [${this._id}] before [${seqId}]#updateUserState: `,
            JSON.stringify(data),
          );
          ...
          EduLogger.info(
            `[userStateUpdated] [${this._id}] after [${seqId}]#updateUserState: `,
            data,
          );
          EduLogger.info(
            `[userStateUpdated] [${this._id}] after [${seqId}]#updateUserState: `,
        ...
        case EduChannelMessageCmdType.userListBatchUpdated: {
          EduLogger.info(
            `[userListBatchUpdated] [${this._id}] before [${seqId}]#userListBatchUpdated: `,
            JSON.stringify(data),
          );
          ...
          if (operator) {
            ...
            const user = MessageSerializer.getChangedUser(data);
            EduLogger.info(
              `[userListBatchUpdated] [${this._id}] after serialized [getChangedUser] `,
              JSON.stringify(user),
            );
            this.updateUserState(user);
            EduLogger.info(
        ...
        case EduChannelMessageCmdType.streamListChanged: {
          ...
          EduLogger.info(
            `[streamListChanged] [${this._id}] before [${seqId}]#updateStreamList: onlineStreams `,
            onlineStreams,
            ' offlineStreams ',
            offlineStreams,
            ' operatorUser ',
  ...
  public async syncFullSequence() {
    ...
    if (this.retryTimer) {
      ...
      this.retryTimer = setTimeout(() => {
        ...
        this.syncFullSequence().then(onSuccess).catch(onFailure);
      }, 2000);
    }
  }
  ...
  private async syncSequence(roomUuid: string, curSeqId: number, count?: number): Promise<any> {
    ...
    EduLogger.info(
      `[EDU-STATE] [syncSequence] [${this._id}], start: [${curSeqId}] acquire, roomUuid: ${roomUuid}, nextId: ${curSeqId}, count: ${count} `,
    );

    let lastId = 0;

    do {
      const { nextId, list } = await this.roomManager.apiService.syncSequence(
        roomUuid,
        curSeqId,
        count,
      );
      EduLogger.info(
        `[EDU-STATE] [syncSequence] [${this._id}], cur_nextId: [${curSeqId}], nex_nextId: [${nextId}]  acquire, roomUuid: ${roomUuid}, nextId: ${curSeqId}, count: ${count} `,
  ...
  public appendBuffer(buffer: SeqData) {
    ...
    EduLogger.info(
      'Raw ChannelMessage, buffer ### ',
      JSON.stringify(this._bufferMap[buffer.seqId]),
    );
  ...
  public upsertLocalUser(user: EduUserAttrs, streamMap?: Record<string, any>) {
    if (this.localUser) {
      ...
      EduLogger.info(
        `EDU-STATE EduDataController: [${this._id}] upsert local user ${user.userUuid}`,
        this.localUser,
        `id: ${this._id}`,
      );
    } else {
      ...
      if (this.localUser.state) {
        ...
        EduLogger.info(
          `EDU-STATE EduDataController: [${this._id}] added local user ${user.userUuid}`,
          this.localUser,
        );
      }
    }
    if (streamMap) {
      ...
      // })
      EduLogger.info(
        `EDU-STATE EduDataController: [${this._id}] update streams local user ${user.userUuid}`,
        this.localUser,
      );
    }
  ...
  public set localUser(v: EduUserData) {
    this._localUser = v;
    EduLogger.info(
      `EDU-STATE EduDataController: [${this._id}]  access localUser: `,
      v,
      ` _localUser: `,
      JSON.stringify(this._localUser),
    );
  }
  ...
  addStreams(
    rawStreams: EduStreamData[],
    operator: OperatorUser,
    cause: CauseType,
    seqId?: number,
  ) {
    // const cachedUserIds: string[] = this._userList
    //   .map((it: EduUserData) => it.user.userUuid)
    //   .concat([this.localUserUuid]);
    const streams = rawStreams.filter(
      (it: EduStreamData) =>
        this._userMap.has(it.stream.userInfo.userUuid) ||
        this.localUserUuid === it.stream.userInfo.userUuid,
    );
    // EduLogger.info(
    //   `[${this._id}] before [${seqId}]#addStreams: `,
    //   ' user List ',
    //   this._userList,
    //   ' stream List ',
    //   this._streamList,
    ...
    for (let newItem of streams) {
      ...
      const targetIdx = this._streamList.findIndex(
        (it: EduStreamData) => it.stream.streamUuid === newStream.streamUuid,
      );
      const targetStream = this._streamList[targetIdx];
      if (targetStream) {
        // EduLogger.info(
        //   `EDU-STATE addStreams: [${this._id}] before [${seqId}]#addStreams: `,
        //   this._userList,
        //   this._streamList,
        //   ' targetStream ',
        //   targetStream,
        ...
        if (this.streamIsOffline(newItem)) {
          // 删除stream
          EduLogger.info(
            `EDU-STATE addStreams: [${this._id}] streamIsOffline: `,
            newItem,
            targetStream,
          );
          ...
          if (this.isLocalStreams(targetStream.stream)) {
            ...
            this.removeLocalStream(
              targetStream.stream.streamUuid,
              operator,
              cause,
              `addStreams[${seqId}]`,
            );
          ...
          // 更新stream
          EduLogger.info(
            `EDU-STATE EduDataController: [${this._id}] update stream addStreams`,
            newItem,
          );
  ...
  removeTargetStream(targetStream: EduStreamData) {
    this._streamList = this._streamList.filter(
      (it) => it.stream.streamUuid !== targetStream.stream.streamUuid,
    );
  }
  ...
  removeLocalStream(
    ...
    EduLogger.info(
      `[EDU-STATE] [${this._id}] seqId: [${seqId}] removeLocalStream remove after, streamUuid `,
      streamUuid,
    );
    let type = 'main';
    ...
    switch (type) {
      case 'main': {
        ...
        if (mainStream) {
          // EduLogger.info(
          //   `[EDU-STATE] [${this._id}] seqId: [${seqId}] removeLocalStream, streamList: `,
          //   JSON.stringify(this._streamList),
          //   'userList: ',
          //   JSON.stringify(this._userList),
          //   ' cachedLocalStreams ',
          //   JSON.stringify(this._cachedLocalStreams),
          // );
          ...
          // EduLogger.info(
          //   `${Date.now()} local-stream-removed local-stream`,
          //   mainStream,
          //   this._userList,
          // );
          // TODO: removed
  ...
  removeStreams(
    streams: EduStreamData[],
    operator: OperatorUser,
    cause: CauseType,
    seqId?: number,
  ) {
    // EduLogger.info(
    //   `[EDU-STATE] [${this._id}] seqId: [${seqId}] before removeStreams in target: `,
    //   ' streams ',
    //   streams,
    //   ' _userList ',
    //   this._userList,
    ...
    for (let newItem of streams) {
      ...
      if (targetStream) {
        EduLogger.info(
          `[EDU-STATE] [${this._id}] seqId: [${seqId}] removeStreams in condition, streamUuid`,
          targetStream.stream.streamUuid,
          ' newItem',
        ...
        EduLogger.info(
          `[EDU-STATE] [${this._id}] seqId: [${seqId}] removeStreams remove after, streamUuid`,
          targetStream,
          ' newItem',
          JSON.stringify(newItem),
          ' streams',
          JSON.stringify(streams),
          this.isLocalStreams(targetStream.stream),
        );
        if (this.isLocalStreams(targetStream.stream)) {
          EduLogger.info(
            `[EDU-STATE] [${this._id}] seqId: [${seqId}] before removeLocalStream, streamUuid`,
            targetStream,
            ' newItem',
            JSON.stringify(newItem),
            ' streams',
    ...
    for (let newTargetItem of list) {
      ...
      if (target) {
        if (this.userIsOffline(newTargetItem)) {
          ...
          // );
          EduLogger.info(
            '[EDU-STATE] remove user in addUserList after ',
          ...
          EduLogger.info(
            '[EDU-STATE] update user in addUserList before',
            JSON.stringify(newTargetItem),
          );
          ...
          if (this.isLocalUser(target.user.userUuid)) {
            this.localUser.updateUser({
              ...
              rtmToken: newTargetItem.hasOwnProperty('rtmToken')
                ? newTargetItem.rtmToken
                : this.localUser.rtmToken,
            });
            this.fire('local-user-updated', {
              user: newTargetItem,
              operator,
  ...
  removeUserList(list: EduUserData[], operator: OperatorUser, cause: CauseType, seqId?: number) {
    ...
    for (let targetItem of list) {
      // const idx = this._userList.findIndex(
      //   (it: EduUserData) => it.user.userUuid === targetItem.user.userUuid,
      // );
      // const target = this._userList[idx];
      ...
      if (target) {
        ...
        // this._userList = this._userList.filter(
        //   (it) => it.user.userUuid !== targetItem.user.userUuid,
        // );
        ...
        EduLogger.info(
          `[EDU-STATE] [${this._id}] seqId: [${seqId}] before removeUserList in target: `,
          ' isLocalUser',
          this.isLocalUser(targetItem.user.userUuid),
          ' list: ',
          JSON.stringify(list),
        );
        this.removeStreams(
        ...
        // EduLogger.info(
        //   `[EDU-STATE] [${this._id}] seqId: [${seqId}] after removeUserList in target: `,
        //   '  targetItem ',
        //   targetItem,
        //   ' userList ',
        //   this._userList,
  ...
  updateUserList(
    ...
    // EduLogger.info(
    //   `[${this._id}] before [${seqId}]#updateUserList: `,
    //   this._userList,
    //   this._streamList,
    // );
    ...
    // EduLogger.info(
    //   `[${this._id}] after [${seqId}]#updateUserList: `,
    //   this._userList,
    //   this._streamList,
    // );
  }
  ...
  updateStreamList(
    ...
    offlineUsers: EduStreamData[],
    operatorUser: OperatorUser,
    cause: CauseType,
    seqId?: number,
  ) {
    // EduLogger.info(
    //   `[${this._id}] before [${seqId}]#updateStreamList: `,
    //   this._userList,
    //   this._streamList,
    // );
    ...
    // EduLogger.info(
    //   `[${this._id}] after [${seqId}]#updateStreamList: `,
    //   this._userList,
    //   this._streamList,
    // );
  }
  ...
  upsertLocalStream(
    ...
    EduLogger.info(
      `[EDU-STATE] upsertLocalStream: [${seqId}] type: ${type}, data: ${JSON.stringify(
        data,
  ...
  setRawUsers(rawUsers: any[]) {
    ...
    const streams = EduStreamData.fromArray(
      rawUsers
