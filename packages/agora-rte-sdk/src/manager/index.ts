...
export class EduManager extends EventEmitter {
  ...
  constructor(config: EduConfiguration) {
    ...
    if (buildOption.platform === 'electron') {
      buildOption.electronLogPath = {
        logPath: this.config.logDirectoryPath
          ? `${this.config.logDirectoryPath}/agorasdk.log`
          : window.logPath || '',
        videoSourceLogPath: this.config.logDirectoryPath
          ? `${this.config.logDirectoryPath}/videosource.log`
          : window.videoSourceLogPath || '',
      };
    ...
    Object.assign(internalEduManagerConfig, {
      appId: this.config.appId,
      sdkDomain: this.config.sdkDomain,
    });
  ...
  async login(userUuid: string) {
    try {
      // REPORT
      rteReportService.initReportUserParams({
        sid: this._sessionId,
        appId: this.config.appId,
        uid: userUuid,
      });
      ...
      rteReportService.reportElapse('init', 'rtm', {
        api: 'login',
        result: true,
      });
  ...
  async _login(userUuid: string) {
    ...
    try {
      ...
      rtmWrapper.on('ConnectionStateChanged', async (evt: any) => {
        ...
        if (
          rtmWrapper.prevConnectionState === 'RECONNECTING' &&
      ...
      rtmWrapper.on('MessageFromPeer', (evt: any) => {
        ...
        switch (cmd) {
          case EduPeerMessageCmdType.peer: {
            EduLogger.info(
              `custom chat message, PeerMessage.${EduPeerMessageCmdType.peer}: `,
              data,
              requestId,
            );
  ...
  createClassroom(params: ClassroomInitParams): EduClassroomManager {
    ...
    this._dataBuffer[params.roomUuid] = new EduClassroomDataController(
      this._classroomMap[params.roomUuid],
    );
